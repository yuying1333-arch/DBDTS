...
namespace VOL.System.Controllers
{
    ...
    public partial class Sys_UserController
    {
        ...
        /// <summary>
        /// 移动端系统登录（无验证码），用于在统一登录页做角色分流
        /// </summary>
        [HttpPost, Route("loginApp"), AllowAnonymous]
        public async Task<IActionResult> LoginApp([FromBody] AppLoginInfo loginInfo)
        {
            ...
            return Json(await Service.Login(req, false));
        }

        /// <summary>
        /// 注册新用户，一律为普通用户权限。密码密文存储。
        /// </summary>
        [HttpPost, Route("register"), AllowAnonymous]
        public async Task<IActionResult> Register([FromBody] RegisterInfo reg)
        {
            if (reg == null)
                return Json(new WebResponseContent().Error("参数不能为空"));
            return Json(await Service.Register(reg));
        }
        ...
        public IActionResult ModifyUserPwd(string password, string userName)
        {
            ...
            if (UserContext.IsRoleIdSuperAdmin(user.Role_Id))
            {
                return Json(webResponse.Error("超级管理员不可修改"));
            }
